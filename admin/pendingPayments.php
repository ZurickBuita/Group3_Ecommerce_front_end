...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['acceptBtn'])) {
        $proposalId = test_input($_POST['acceptBtn']);
        $query = "UPDATE `project_proposal` SET `status`='2' WHERE `proposalId`='$proposalId'";
        $query_run = mysqli_query($conn, $query);

        if ($query_run) {
            echo "<script>console.log('Project rejected successfully');</script>";
            $alertMssg =
        ...
        $notifType = "Accepted your project proposal";
        $query = "INSERT INTO `notification`(`userId`, `notificationText`, `notificationType`, `proposalId`) VALUES ('$userId','$notifType','$userType','$proposalId')";

        if (mysqli_query($conn, $query)) {
            echo "<script>console.log('$notifType')</script>";
        }
    }

    if (isset($_POST['rejectBtn'])) {
        $proposalId = test_input($_POST['rejectBtn']);
        ...
        $notifType = "Rejected your project proposal";
        $query = "INSERT INTO `notification`(`userId`, `notificationText`, `notificationType`, `proposalId`) VALUES ('$userId','$notifType','$userType','$proposalId')";
    ...
    if (isset($_POST['mssgBtn'])) {
        $_SESSION['serviceId'] = $_POST['mssgBtn'];
        $_SESSION['previousDir'] = "pendingPayments.php";

        header("Location: projectMssg.php");
    }

    if (isset($_POST['viewProofOfPayment'])) {
        $_SESSION['proposalId'] = $_POST['proposalId'];
        $_SESSION['customerId'] = $_POST['customerId'];
        $_SESSION['prevDir'] = "pendingPayments.php";
        
        header("Location: viewProofOfPayment.php");
    }
}
...
<body class="position-relative">
    ...
    <main id="main" class="main pb-5 mb-5">
        ...
        INNER JOIN category ON project_proposal.projectCategory = category.categoryId) WHERE `status` = '1';";
        $query_run = mysqli_query($conn, $query);
            ...
            <div class="card-footer text-body-secondary">
                <div class="row g-3 d-flex align-items-center justify-content-between">
                    ...
                    <div class="col-md-6 col-sm-12 text-end">
                        <form class="d-inline" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                            method="POST">
                            <button type="submit" name="rejectBtn" class="rejectBtn btn btn-danger"
                                value="<?= $data['proposalId'] ?>">Reject</button>
                        </form>
                        <form class="d-inline" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                            method="POST">
                            <button type="submit" class="acceptBtn btn btn-success" value="<?= $data['proposalId'] ?>"
                                name="acceptBtn">Accept</button>
                        </form>
                        <form class="d-inline" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                            method="POST">
                            <button type="submit" class="acceptBtn btn btn-primary" value="<?= $data['proposalId'] ?>"
                                name="mssgBtn">Chat with client</button>
                        </form>
                        ...
                        <form class="d-inline" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>"
                            ...
                            <button type="submit" class="btn btn-info" name="viewProofOfPayment">Proof of
                                Payment</button>
                        </form>
